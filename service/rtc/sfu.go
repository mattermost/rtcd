...
func (s *Server) InitSession(cfg SessionConfig, closeCb func() error) error {
	...
	peerConn.OnTrack(func(remoteTrack *webrtc.TrackRemote, receiver *webrtc.RTPReceiver) {
		...
		s.metrics.IncRTPTracks(us.cfg.GroupID, "in", getTrackKind(remoteTrack.Kind()))
		defer func() {
			...
			s.metrics.DecRTPTracks(us.cfg.GroupID, "in", getTrackKind(remoteTrack.Kind()))
		}()
		...
		if trackMimeType == rtpAudioCodec.MimeType {
			...
			// TODO: enforce video track stream ID check
