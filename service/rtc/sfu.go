...
func (s *Server) initSettingEngine(loggerFactory logging.LoggerFactory) (webrtc.SettingEngine, error) {
	...
	if err != nil {
		...
		// Convert to new ICEAddressRewriteRules API (replaces deprecated SetNAT1To1IPs)
		if err := sEngine.SetICEAddressRewriteRules(webrtc.ICEAddressRewriteRule{
			External:        pairs,
			AsCandidateType: webrtc.ICECandidateTypeHost,
			// Mode defaults to ICEAddressRewriteReplace for host candidates
		}); err != nil {
			return webrtc.SettingEngine{}, fmt.Errorf("failed to set ICE address rewrite rules: %w", err)
		}
	}
		...
		if trackMimeType == rtpAudioCodec.MimeType {
			...
			for {
				...
				if err := outAudioTrack.WriteRTP(packet); err != nil && !errors.Is(err, io.ErrClosedPipe) {
					...
					s.log.Warn("failed to write RTP packet",
						mlog.Err(err), mlog.String("sessionID", us.cfg.SessionID))
					continue
				}
